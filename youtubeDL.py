...
def main(url, directory):
    # directory = "E:\\Staging"
    # directory = os.getcwd() if len(argss) != 3 else argss[2]
...
if __name__ == '__main__':
    # if len(argss) < 2 or len(argss) > 3:
    #     print('USAGE: python ytPlaylistDL.py playlistURL OR python ytPlaylistDL.py playlistURL destPath')
    #     exit(1)
    # else:
    if True:
        argss = [""]
        argss.append("https://www.youtube.com/playlist?list=PLBsP89CPrMePNK7yIxcyRdiIFentQVHra")
        argss.append("E:\\Staging\\Youtube DL")
        url = argss[1]
        directory = os.getcwd() if len(argss) != 3 else argss[2]
